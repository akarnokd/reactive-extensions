...
namespace akarnokd.reactive_extensions
{
    ...
    internal sealed class MaybeRepeatWhen<T, U> : IObservable<T>
    {
        readonly IMaybeSource<T> source;

        readonly Func<IObservable<object>, IObservable<U>> handler;

        public MaybeRepeatWhen(IMaybeSource<T> source, Func<IObservable<object>, IObservable<U>> handler)
        {
            this.source = source;
        ...
        internal sealed class RepeatWhenObserver : MaybeRedoWhenObserver<T, U, object>
        {
            readonly IObserver<T> downstream;

            public RepeatWhenObserver(IObserver<T> downstream, IMaybeSource<T> source, IObserver<object> terminalSignal) : base(source, terminalSignal)
            {
