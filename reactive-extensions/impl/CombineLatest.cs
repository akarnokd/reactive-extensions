...
namespace akarnokd.reactive_extensions
{
    ...
    internal sealed class CombineLatest<T, R> : IObservable<R>
    {
        ...
        public CombineLatest(IObservable<T>[] sources, Func<T[], R> mapper, bool delayErrors)
        {
        ...
        sealed class CombineCoordinator : IDisposable
        {
            ...
            internal void Subscribe(IObservable<T>[] observables)
            {
                var o = observers;
                var n = o.Length;
                ...
                for (int i = 0; i < n; i++)
                {
                    ...
                    if (inner != null)
                    {
                        inner.OnSubscribe(observables[i].Subscribe(inner));
                    }
                }
            }
