...
namespace akarnokd.reactive_extensions
{
    ...
    internal sealed class ObservableSourceMergeMany<T> : IObservableSource<T>
    {
        readonly IObservableSource<IObservableSource<T>> source;
        ...
        public void Subscribe(ISignalObserver<T> observer)
        {
            ...
            {
                source.Subscribe(new MergeManyMaxObserver(observer, delayErrors, capacityHint));
            }
            else
            {
                source.Subscribe(new MergeManyLimitedObserver(observer, delayErrors, maxConcurrency, capacityHint));
            }
        }

        internal abstract class MergeManyBaseObserver : BaseSignalObserver<IObservableSource<T>, T>
        {
            ...
            internal MergeManyBaseObserver(ISignalObserver<T> downstream, bool delayErrors, int capacityHint) : base(downstream)
            {
                this.delayErrors = delayErrors;
                this.capacityHint = capacityHint;
        ...
        {
            readonly ConcurrentQueue<IObservableSource<T>> sources;

            readonly int maxConcurrency;

            bool done;

            internal MergeManyLimitedObserver(ISignalObserver<T> downstream, bool delayErrors, int maxConcurrency, int capacityHint) : base(downstream, delayErrors, capacityHint)
            {
                this.maxConcurrency = maxConcurrency;
                this.sources = new ConcurrentQueue<IObservableSource<T>>();
            }
            ...
            public override void OnNext(IObservableSource<T> value)
            {
        ...
        internal sealed class MergeManyMaxObserver : MergeManyBaseObserver
        {
            ...
            internal MergeManyMaxObserver(ISignalObserver<T> downstream, bool delayErrors, int capacityHint) : base(downstream, delayErrors, capacityHint)
            {
