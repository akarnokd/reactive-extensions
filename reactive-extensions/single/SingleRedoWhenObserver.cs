...
{
    /// <summary>
    /// Base class for supporting retrying/repeating single sources.
    /// </summary>
    /// <typeparam name="T">The success type of the source.</typeparam>
    /// <typeparam name="U">The signal type of the handler to trigger re-subscription.</typeparam>
    /// <typeparam name="X">The signal type to trigger the handler.</typeparam>
    /// <remarks>Since 0.0.13</remarks>
    internal abstract class SingleRedoWhenObserver<T, U, X> : ISingleObserver<T>, IDisposable
    {
        ...
        public SingleRedoWhenObserver(ISingleSource<T> source, IObserver<X> terminalSignal)
        {
            this.source = source;
        ...
        {
            readonly SingleRedoWhenObserver<T, U, X> parent;

            IDisposable upstream;

            public RedoObserver(SingleRedoWhenObserver<T, U, X> parent)
            {
                this.parent = parent;
            }
