...
namespace akarnokd.reactive_extensions
{
    ...
    internal sealed class SingleFlatMapObservable<T, R> : IObservable<R>
    {
        readonly ISingleSource<T> source;

        readonly Func<T, IObservable<R>> mapper;

        public SingleFlatMapObservable(ISingleSource<T> source, Func<T, IObservable<R>> mapper)
        {
            this.source = source;
            this.mapper = mapper;
        }
        ...
        sealed class FlatMapEnumerableObserver : ISingleObserver<T>, IObserver<R>, IDisposable
        {
