...
namespace akarnokd.reactive_extensions
{
    ...
    internal sealed class SingleFlatMapMany<T, R> : IObservable<R>
    {
        ...
        public SingleFlatMapMany(IObservable<T> sources, Func<T, ISingleSource<R>> mapper, bool delayErrors, int maxConcurrency)
        {
        ...
        sealed class FlatMapLimitedCoordinator : SingleMergeCoordinator<R>, IObserver<T>
        {
            readonly Func<T, ISingleSource<R>> mapper;
            ...
            internal FlatMapLimitedCoordinator(IObserver<R> downstream, Func<T, ISingleSource<R>> mapper, 
        ...
        sealed class FlatMapAllCoordinator : SingleMergeCoordinator<R>, IObserver<T>
        {
            readonly Func<T, ISingleSource<R>> mapper;
            ...
            internal FlatMapAllCoordinator(IObserver<R> downstream, Func<T, ISingleSource<R>> mapper, bool delayErrors) : base(downstream, delayErrors)
            {
                this.mapper = mapper;
            ...
            public void OnNext(T item)
            {
                ...
                try
                {
                    src = RequireNonNullRef(mapper(item), "The mapper returned a null ISingleSource");
                }
                catch (Exception ex)
                {
                    DisposableHelper.Dispose(ref upstream);
