...
namespace akarnokd.reactive_extensions
{
    ...
    internal sealed class SingleMergeEnumerable<T> : IObservable<T>
    {
        ...
        public SingleMergeEnumerable(IEnumerable<ISingleSource<T>> sources, bool delayErrors, int maxConcurrency)
        {
        ...
        sealed class MergeLimitedCoordinator : SingleMergeCoordinator<T>
        {
            readonly int maxConcurrency;
            ...
            internal MergeLimitedCoordinator(IObserver<T> downstream, IEnumerator<ISingleSource<T>> sources, bool delayErrors, int maxConcurrency) : base(downstream, delayErrors)
            {
                this.sources = sources;
                this.maxConcurrency = maxConcurrency;
            }
        ...
        sealed class MergeAllCoordinator : SingleMergeCoordinator<T>
        {
            int active;
            ...
            internal void Subscribe(IEnumerator<ISingleSource<T>> sources)
            {
                Interlocked.Increment(ref active);
