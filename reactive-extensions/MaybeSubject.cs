...
{
    /// <summary>
    /// A maybe-based, hot subject that multicasts the success or termination event
    /// it receives through its maybe observer API surface.
    /// </summary>
    /// <remarks>Since 0.0.9</remarks>
    public sealed class MaybeSubject<T> : IMaybeSource<T>, IMaybeObserver<T>, ISubjectExtensions
    {
        ...
        /// <summary>
        /// Construct an active (non-terminated) maybe subject.
        /// </summary>
        ...
        /// this maybe subject.</param>
        public MaybeSubject(bool refCount = false)
        {
        ...
        {
            return Volatile.Read(ref observers) == TERMINATED && error == null && !hasValue;
        }
        ...
        /// <summary>
        /// Returns true if this maybe subject was terminated with
        /// a success value.
        /// </summary>
        /// <returns>True if this maybe subject was terminated with a
        /// success value.</returns>
        public bool HasValue()
        {
            return Volatile.Read(ref observers) == TERMINATED && hasValue;
        }

        /// <summary>
        /// Tries to retrieve the success value of this maybe subject
        /// if it has been terminated with one.
        /// </summary>
        /// <param name="value">The value read out.</param>
        /// <returns>True if this maybe subject terminated with a value, false if
        /// it is still running.</returns>
        public bool TryGetValue(out T value)
        {
            if (Volatile.Read(ref observers) == TERMINATED && hasValue)
            {
                value = this.value;
                return true;
            }
        ...
        {
            internal readonly IMaybeObserver<T> downstream;

            MaybeSubject<T> parent;

            public InnerDisposable(IMaybeObserver<T> downstream, MaybeSubject<T> parent)
            {
